...
export default function Footer({ data, navLinks, socialLinks }) {
  ...
  const socials = (data?.socialLinks?.length ? data.socialLinks : socialLinks) || [];
  const brandImage = data?.brandImage || data?.logo || '/loog-hcb.png';
  ...
  const copyright = data?.copyright || `© ${new Date().getFullYear()} Himesh Bhattarai. All rights reserved.`;

  return (
    <footer className="border-t border-[--line] bg-[--page-bg] text-[--muted]">
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 xl:px-16 py-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
